...
module.exports = {
    data: new SlashCommandBuilder()
            .setName('janken')
            .setDescription('Earn money with janken!')
            .addNumberOption((option) => option
                .setName('bet')
                .setDescription('what u wanna bet')
                .setRequired(true),),
    async execute(client, interaction) {
        ...
        //回覆
        interaction.reply({ embeds: [buttonEmbed], components: [buttonRow], ephemeral:true});
        ...
        collector.on('collect', async collected => {
            ...
            let winner = 0;
            if(playerChoice == 0) {
                if(botChoice == 1) winner = 1; else if(botChoice == 2) winner = 2;
            }
            if(playerChoice == 1) {
                if(botChoice == 0) winner = 2; else if(botChoice == 2) winner = 1;
            }
            if(playerChoice == 2) {
                if(botChoice == 0) winner = 1; else if(botChoice == 1) winner = 2;
            }
            //從結果計算獲得/失去的 money
            let earnings = 0;
            if(winner == 1) earnings += interaction.options.getNumber('bet');
            else if(winner == 2) earnings -= interaction.options.getNumber('bet');
